...
const requiredSchemaVersion = '20260904020000_r2_transaction_safe_booking';
...
export async function GET(req: NextRequest) {
  ...
  if (isLocalMode) {
    return NextResponse.json({
      ...
      checks: { database: 'LOCAL_ONLY', migration_state: 'LOCAL_ONLY', orchestration_repository: 'LOCAL_ONLY', worker: 'UNAVAILABLE', timers: 'LOCAL_ONLY', outbox: 'LOCAL_ONLY', recovery: 'LOCAL_ONLY' },
  ...
  try {
    const [schemaRows, flowRows, workerRows, timerRows, outboxRows, recoveryRows] = await Promise.all([
