...
export default function PayPage() {
  ...
  return <AppShell persona="customer" wide={false}>
    ...
    {loading ? <div className="kj-state">Loading checkout…</div> : !booking || !quote ? <div className="kj-state kj-state--error">{error || 'Booking not found.'}</div> : expired ? <div className="kj-state kj-state--error"><strong>Your five-minute reservation expired.</strong><p>The original slot has been released. If a payment later confirms, Kajola protects the money and opens recovery instead of displacing another customer.</p><a className="kj-btn kj-btn--secondary" href="/discovery">Choose another time</a></div> : <div className="kj-checkout-grid">
