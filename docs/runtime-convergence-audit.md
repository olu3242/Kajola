...
Supabase CLI 2.109.1 is installed, but repeated `supabase projects list` execution returned `Unauthorized`, including after the user confirmed rotating the disclosed replacement database credential. A simultaneous `git fetch origin` succeeded, isolating the stop to Supabase CLI authentication/authorization rather than general network reachability. This is classified as `PROJECT_AUTHORIZATION_FAILURE`; project link, pooler transport, Postgres authentication, migration history, dry-run, and application were not attempted. The newly rotated credential was not provided to or used by this run. The active runtime also has none of the required connected Supabase/runtime variables present. Fresh local gates passed: 7/7 workspace typechecks, 83/83 Vitest assertions, 20/20 migration validations, web lint, 6/6 build tasks, and 38/38 Playwright scenarios. Playwright ran in local mode and is not live durability or RLS evidence.
