...
export async function GET(req: NextRequest) {
  ...
  if (!isLocalMode) {
    try {
      ...
      const [events, audits, ledger, timers, notifications, workers, deadLetters, recoveryCases, bookingTransitions, settlements, reconciliation, agentActions] = await Promise.all([
        ...
        durableRequest<unknown[]>('/rest/v1/settlements?status=in.(ELIGIBLE,QUEUED,PROCESSING,FAILED,RETRY_REQUIRED,HELD)&select=id&limit=1000'),
        durableRequest<unknown[]>('/rest/v1/reconciliation_exceptions?state=in.(OPEN,INVESTIGATING)&select=id&limit=1000'),
        durableRequest<unknown[]>('/rest/v1/agent_action_requests?status=in.(REQUESTED,FAILED)&select=id&limit=1000'),
      ]);
      durableEvidence = { events: events.length, audits: audits.length, booking_transition_audits: bookingTransitions.length, ledger_entries: ledger.length, scheduled_workflows: timers.length, notifications: notifications.length, worker_heartbeats: workers, open_dead_letters: deadLetters.length, recovery_backlog: recoveryCases.length, settlement_backlog: settlements.length, reconciliation_backlog: reconciliation.length, agent_action_backlog: agentActions.length, flow_repository: 'supabase' };
    } catch (error) {
